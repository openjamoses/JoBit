...
public class Pick_Names {
	...
	Row row;
        ...
        for (j=0; j< spreadsheet.getLastRowNum()+1; ++j) {//To loop thru the rows in a sheet
           ...
           cell = row.getCell(1); //forks are in the eighth column...
           switch (cell.getCellType()){
               //Checking for strings values inthe cells..
               case Cell.CELL_TYPE_STRING:
                   if (!cell.getStringCellValue().equals("") && !cell.getStringCellValue().equals("NAME")){
                      // adding the call value to the arraylist called list 
