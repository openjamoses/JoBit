 ...
 * @author john
 */
public class Pick_Commits {
    public ArrayList<String> pickDatas(String file,int loop) throws Exception{
        OpenFileName fname;
        int j,x;
	...
	Row row;
        ...
        for (j=0; j< spreadsheet.getLastRowNum()+1; ++j) {//To loop thru the rows in a sheet
           ...
           cell = row.getCell(8); //forks are in the eighth column...
           switch (cell.getCellType()){
               //Checking for strings values inthe cells..
               case Cell.CELL_TYPE_STRING:
                   if (!cell.getStringCellValue().equals("") && !cell.getStringCellValue().equals("Name/email/login/Location/Created_at/Updated_at/Public_repos/Public_gists/Followers/Following/Commits_Changed_Added_Deleted")){
